...
class VoicePolicy:
    """
    高层语音意图到 Runtime 底层请求字段的确定性映射。

    核心入参:
        intent: Runtime 语义标签，用于日志、状态和未来 UI 展示。
        action: Runtime 显式队列控制动作，决定当前播报是否入队、替换、清队或打断。
        interrupt: 兼容底层协议的显式打断布尔值，urgent 需要同时置位以确保调用链一致。
    ...
    def to_request_kwargs(self) -> dict[str, str | bool]:
        """
        转换为 VoiceClient.say 可直接使用的请求字段。

        核心入参:
            本方法不接收额外参数，直接读取当前策略字段。

        预期输出:
            返回 intent/action/interrupt 三个底层协议字段。
...
DEFAULT_VOICE_POLICIES: dict[str, VoicePolicy] = {
    ...
    "info": VoicePolicy(intent="info", action="enqueue"),

    # 进度播报允许覆盖尚未播放的旧 progress，业务意图是避免用户听到过期过程。
    "progress": VoicePolicy(intent="progress", action="replace_pending"),

    # warning 用于提醒用户关注新的风险或注意点，业务意图是清理过期进度后优先播报提醒，但不强制打断当前播放。
    "warning": VoicePolicy(intent="warning", action="clear_pending_then_enqueue"),

    # 紧急播报必须立即打断，业务意图是让错误或风险信息不要滞后于普通队列。
    "urgent": VoicePolicy(intent="urgent", action="interrupt", interrupt=True),

    # 完成播报清理旧待播内容再收尾，业务意图是让最终结果优先于过期进度。
    "done": VoicePolicy(intent="done", action="clear_pending_then_enqueue"),
}
...
def resolve_voice_policy(name: str) -> VoicePolicy:
    ...
    预期输出:
        返回对应 VoicePolicy，供 Client 或 CLI 组装底层请求。
