...
class SenseVoiceDecoder:
    def __init__(self, decoder_path: str, onnx_provider="cpu", pad_to: int = 30):
        ...
        logger.info(f"[Decoder] 正在初始化 ONNX 会话 (Provider: {self.onnx_provider})...")
        ...
        logger.info(f"[Decoder] 输入精度: {self.input_dtype}")
    ...
    def warmup(self):
        ...
        logger.info(f"[Decoder] DML 推理模式：正在使用形状为 {dummy_enc.shape} 的数据进行预热...")
        self.session.run(None, {"enc_out": dummy_enc})
        logger.info("[Decoder] DML 预热完成。")
