...
def load_audio_numpy(audio_path, sample_rate=16000, start_second=None, duration=None):
    ...
    offset = start_second if start_second is not None else 0.0
    if duration is not None and duration <= 0:
        duration = None

    # librosa.load 参数说明:
    # - sr=sample_rate: 自动重采样到目标采样率 (默认使用高质量的 resampy/soxr)
    # - mono=True: 自动混合多声道为单声道 (默认行为，取平均值)
    # - offset: 起始时间 (秒)
    # - duration: 读取时长 (秒)，为 None 时读取剩余所有
...
class NumPyMelExtractor:
    ...
    def __init__(self, sr=16000, n_fft=400, n_mels=80, f_min=20, f_max=8000):
        ...
        fb = np.maximum(
            0,
            np.minimum(
                (-1.0 * slopes[:, :-2]) / f_diff[:-1], slopes[:, 2:] / f_diff[1:]
            ),
        )
