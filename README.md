...
- 在任务刚进入执行态、开始明显耗时步骤、发现风险、出现阻塞、阶段完成、整体完成这些关键状态变化时，必须按该 skill 执行播报。
- 任务从“收到请求”切换到“开始实际处理”时，必须先播一条起手语音，再进入后续工具调用、长分析或文件修改；这条起手播报默认使用 `info`，文案要明确表达“我准备开始处理什么”或“我现在开始做什么”。
- 如果准备发送一条中间文本更新，而这条文本已经包含新的执行状态、策略切换、数据源切换、样本已拿到、阶段已切换或下一步已改变，就必须先做对应播报，不能只发文字不发语音。
- 一次工具批次、检索批次或分析批次结束后，只要已经形成新的可同步状态，就视为新的播报节点；文本更新与语音播报不是二选一。
...
- 起手 `info` 播报不能用“别急”“再等等”“我还在看”这种默认用户已经知道任务开始了的句子起手；第一句就要让用户听懂“现在正式开始处理了”。
```
...
- `info`
  补充说明当前情况或下一步，也可作为任务正式开始时的起手播报；策略切换、数据源切换、检索路径调整这类新的中间执行状态，通常也使用 `info`
- `progress`
  表示任务正在推进；长步骤中也允许作为 20 到 30 秒窗口内的心跳补播语义；样本拿到、开始聚类、开始整理这类“已经进入下一段处理”的中间状态，通常也使用 `progress`
- `warning`
  表示发现风险，但任务还可以继续
- `urgent`
  表示当前出现阻塞，必须优先处理
