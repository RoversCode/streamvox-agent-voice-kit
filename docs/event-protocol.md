...
```bash
streamvox-runtime roles register assistant_voice --audio-path ./reference.wav --prompt-text "这是参考音频对应的文本"
```

### 二进制上传注册

```bash
streamvox-runtime roles register assistant_voice --audio-file ./reference.wav --set-default
```
